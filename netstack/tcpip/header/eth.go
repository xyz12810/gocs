...
const (
	...
	// (all bits set to 0).
	unspecifiedEthernetAddress = LinkAddress("\x00\x00\x00\x00\x00\x00")
...
const (
	...
	// capture all traffic.
	EthernetProtocolAll NetworkProtocolNumber = 0x0003

	// EthernetProtocolPUP is the PARC Universial Packet protocol ethertype.
	EthernetProtocolPUP NetworkProtocolNumber = 0x0200
)
...
var Ethertypes = []NetworkProtocolNumber{
	EthernetProtocolAll,
	EthernetProtocolPUP,
}

// SourceAddress returns the "MAC source" field of the ethernet frame header.
func (b Ethernet) SourceAddress() LinkAddress {
	return LinkAddress(b[srcMAC:][:EthernetAddressSize])
}

// DestinationAddress returns the "MAC destination" field of the ethernet frame
// header.
func (b Ethernet) DestinationAddress() LinkAddress {
	return LinkAddress(b[dstMAC:][:EthernetAddressSize])
}

// Type returns the "ethertype" field of the ethernet frame header.
func (b Ethernet) Type() NetworkProtocolNumber {
	return NetworkProtocolNumber(binary.BigEndian.Uint16(b[ethType:]))
}
...
// ethernet address.
func IsValidUnicastEthernetAddress(addr LinkAddress) bool {
