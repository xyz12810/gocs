...
//
// The initial checksum must have been computed on an even number of bytes.
func ChecksumVV(vv VectorisedView, initial uint16) uint16 {
...
//
// The initial checksum must have been computed on an even number of bytes.
func ChecksumVVWithOffset(vv VectorisedView, initial uint16, off int, size int) uint16 {
	odd := false
	sum := initial
...
func PseudoHeaderChecksum(protocol TransportProtocolNumber, srcAddr Address, dstAddr Address, totalLen uint16) uint16 {
