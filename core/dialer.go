...
func (m *MessageDialer) Dial(network, address string) (c net.Conn, err error) {
	...
	err = conn.Connect()
	if err == nil {
		c = NewHashConn(conn, strings.Contains(ShowAddress, strings.Split(address, ":")[0]), "message")
...
func (m *MessageDialer) Write(b []byte) (n int, err error) {
	// fmt.Printf("MessageDialer Write to %v\n", b)
	head := len(m.Header)
	if len(b) < head+9 {
	...
	DebugLog("MessageDialer receive %v cmd with %v data by id %v", b[head+8], len(b), cid)
	m.connsLck.RLock()
	if m.closed {
		err = fmt.Errorf("closed")
		m.connsLck.RUnlock()
...
func (m *MessageDialer) Read(b []byte) (n int, err error) {
	...
	// if err == nil {
	// 	DebugLog("MessageDialer send %v data", n)
	// }
	n, err = m.rawRead(b)
	// if err == nil {
	// 	fmt.Printf("MessageDialer send %v data %v", n, b[:n])
	// }
	return
}
...
func (m *MessageDialer) rawRead(b []byte) (n int, err error) {
	...
	if len(data) > len(b) {
		err = fmt.Errorf("buffer is too small for %v, expect %v", len(b), len(data))
		return
	}
	n = copy(b, data)
	return
